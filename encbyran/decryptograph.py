...
def decryptograph(file_to_decrypt: str, decryptor: str) -> None:
    ...
    :return: Creates a file decrypted using the provided keys.
    """
    with open(file_to_decrypt, 'rt', encoding='utf-8') as encrypted, open(decryptor, 'rt', encoding='utf-8') as decryptor, open(f"decrypted-{file_to_decrypt.rstrip('.txt').lstrip('encrypted-')}.txt", 'w+', encoding='utf-8') as decrypted:
